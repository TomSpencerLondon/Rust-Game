...
impl GameMessage {
    pub(crate) fn new(contents: String) -> GameMessage {
...
impl GameState {
    ...
    fn pickup(self, index: usize) -> GameState {
        let item = self.area.get_from_inventory(&index);
        GameState {
            last_message: GameMessage::new(format!("You pickup the {}", item.name)),
            inventory: self.inventory.with(item.clone()),
    ...
    pub(crate) fn process(self, input: String) -> Game {
        ...
        match inputs[0] {
            "look" => {
                Game::Running(self.look())
            }
            "pickup" => {
                let index = inputs[1].parse::<usize>().unwrap() - 1;
                Game::Running(self.pickup(index))
            }
            "inventory" => {
            ...
            "use" => {
                ...
                match (&item.name, &item.item_type) {
                    (name , ItemType::Potion) => Game::Running(GameState {
                    ...
                    (name, ItemType::Poison) => Game::Running(GameState {
                        last_message: GameMessage { contents: String::from(String::from("You drink the poison.")) },
                        inventory: self.inventory.with(item),
                        health: self.health - 1,
                        ..self
                    })
